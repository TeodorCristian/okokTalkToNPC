...
Citizen.CreateThread(function()
	...
	while true do
		...
		if inZone and not hasEntered then
			if Config.UseOkokTextUI then
				exports['okokTextUI']:Open('[E] Vorbeste cu '..name, 'darkblue', 'left') 
			end
			hasEntered = true
...
RegisterNetEvent("bunker:buletin")
AddEventHandler("bunker:buletin", function()
	exports['okokNotify']:Alert("Tutoriale", "Buletinul il poti face chiar aici, la usa primariei !", 10000, 'info')
end)

RegisterNetEvent("bunker:imbracaminte")
AddEventHandler("bunker:imbracaminte", function()
	exports['okokNotify']:Alert("Tutoriale", "Te duci la orice magazin de haine si iti customizezi caracterul absolut cum vrei tu.", 10000, 'info')
end)

RegisterNetEvent("bunker:tatoo")
AddEventHandler("bunker:tatoo", function()
	exports['okokNotify']:Alert("Tutoriale", "Te duci, iti inchiriezi o masina si dai drumul la roleplay !", 10000, 'info')
end)

RegisterNetEvent("bunker:rules")
AddEventHandler("bunker:rules", function()
	exports['okokNotify']:Alert("Tutoriale", "Regulamentul il poti gasi la noi pe discord.", 10000, 'info')
end)


RegisterNetEvent("bunker:linkDiscord")
AddEventHandler("bunker:linkDiscord", function()
	exports['okokNotify']:Alert("Tutoriale", "Discord-ul nostru este : discord.io/alaskarorp", 10000, 'info')
end)


RegisterNetEvent("bunker:cumDonez")
AddEventHandler("bunker:cumDonez", function()
	exports['okokNotify']:Alert("Tutoriale", "DONATIILE SE FAC DOAR LA : 1 | Mario , 2 | k1nG ,3 | Minato. Nu raspundem donatiilor facute altcuiva si banii nu se returneaza", 10000, 'info')
end)

RegisterNetEvent('pescar:undeVand')
AddEventHandler('pescar:undeVand', function()
	exports['okokNotify']:Alert("Tutoriale", "text", 10000, 'info')
	SetNewWaypoint(-695.40405273438,5802.326171875)
end)